...
-- operator has a precedence of 7 which is greater than the addition operator's
-- precedence of 6. The expression (8 + 7) * 9 overrides the multiplication
-- operator's precedence using parentheses and evaluates to 135.

-- 2. The expressions are equivalent because parentheses in (b) are implied
-- because the precedence of the multiplication operator is higher than that of
...
-- 4. 100 `div` 3 and 100 / 3 will not return the same result when
-- evaluated. The div function performs integer division (rounding down).
